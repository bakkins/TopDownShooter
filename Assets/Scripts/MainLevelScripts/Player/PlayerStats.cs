...
public class PlayerStats : MonoBehaviour
{
    ...
    public DeathPanelManager deathManager;
    ...
    void Die()
    {
        Debug.Log("💀 Player Died! Calculating Final Score...");
        ...
        if (gm != null)
        {
            // We call gm.GetCurrentWave() because the wave logic is in the GameManager
            finalScore = (gm.GetCurrentWave() * 100) + gm.coins;
        }
        else
        {
            Debug.LogWarning("GameManager not found! Score will be 0.");
        }

        // Trigger the Death Panel
        if (deathManager != null)
        {
            deathManager.ToggleDeathPanel(finalScore);
        }
        else
        {
            Debug.LogError("DeathManager reference is missing on PlayerStats!");
        }
    }
}